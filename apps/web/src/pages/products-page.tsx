...
export function ProductsPage() {
  ...
  return <div className="page-stack">
    ...
    <Card className="data-card"><div className="data-toolbar"><div className="search-control"><MagnifyingGlass size={16} aria-hidden="true" /><Input value={search} onChange={(event) => { setSearch(event.target.value); setCursors([undefined]); }} placeholder="Search name, SKU, GTIN, or brand" aria-label="Search products by name, SKU, GTIN, or brand" /></div><div className="toolbar-actions"><select className="select-control compact-select" value={status} onChange={(event) => { setStatus(event.target.value); setCursors([undefined]); }} aria-label="Filter by status"><option value="">All statuses</option><option value="DRAFT">Draft</option><option value="READY">Ready</option><option value="ACTIVE">Active</option><option value="ARCHIVED">Archived</option></select></div></div>
      ...
      <div className="table-footer"><span>{products.data?.items.length ?? 0} products on this page</span><div><Button variant="secondary" size="icon" aria-label="Previous page" disabled={cursors.length === 1} onClick={() => setCursors((value) => value.slice(0, -1))}><CaretLeft size={15} /></Button><span>Page {cursors.length}</span><Button variant="secondary" size="icon" aria-label="Next page" disabled={!products.data?.page.hasMore} onClick={() => setCursors((value) => [...value, products.data?.page.nextCursor])}><CaretRight size={15} /></Button></div></div>
    </Card>
  </div>;
}
